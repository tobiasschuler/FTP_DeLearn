...
\q Different \emph{types of task} that RNNs can address, with one example application each.
\begin{answerbox}
Classified by the input/output cardinality (sequence length need not match the input length):
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Type} & \textbf{Mapping} & \textbf{Example application} \\
\midrule
One-to-many  & 1 input $\to$ sequence      & Image captioning (image $\to$ caption) \\
Many-to-one  & sequence $\to$ 1 output     & Sentiment / sequence classification \\
Many-to-many (aligned) & sequence $\to$ sequence (same length) & POS tagging, named-entity recognition \\
Many-to-many (seq2seq) & sequence $\to$ sequence (diff.\ length) & Machine translation, speech recognition \\
\bottomrule
\end{tabular}
\end{center}
(\textbf{One-to-one} is the degenerate non-recurrent case, e.g.\ plain image classification.) The shared point: a recurrent hidden state carries context across time, so the network handles variable-length, order-dependent data.
\end{answerbox}

\q Number of trainable parameters of a \textbf{two-layer Elman RNN + softmax}: hidden dims $32$ and $64$, $10$ softmax classes, sequences of length $100$, each element a vector of dimension $30$.
\begin{answerbox}
Sequence length ($100$) is \emph{irrelevant} -- weights are shared across time. Per Elman layer with input dim $d$, hidden dim $h$: $W_{xh}(h{\times}d)+W_{hh}(h{\times}h)+b(h)=h(d+h+1)$.
\[
\begin{aligned}
\text{Layer 1 } (d{=}30,\,h{=}32):\;& 32(30+32+1)=32\cdot 63 = 2016\\
\text{Layer 2 } (d{=}32,\,h{=}64):\;& 64(32+64+1)=64\cdot 97 = 6208\\
\text{Softmax } (64\to 10):\;& 64\cdot 10 + 10 = 650\\
\hline
\textbf{Total}:\;& 2016+6208+650 = \mathbf{8874}
\end{aligned}
\]
The second layer's input dim is the first layer's hidden dim ($32$), not the raw input dim ($30$).
\end{answerbox}

\q Same as above but a \textbf{two-layer LSTM + softmax} (hidden dims $32,64$; $10$ classes; length $100$; element dim $30$).
\begin{answerbox}
An LSTM has \textbf{4 gate sets} (input, forget, cell-candidate, output), each with its own $W_{xh},W_{hh},b$, so a layer costs $4\,h(d+h+1)$ -- exactly $4\times$ the Elman count.
\[
\begin{aligned}
\text{Layer 1}:\;& 4\cdot 2016 = 8064\\
\text{Layer 2}:\;& 4\cdot 6208 = 24832\\
\text{Softmax}:\;& 650\\
\hline
\textbf{Total}:\;& 8064+24832+650 = \mathbf{33546}
\end{aligned}
\]
\end{answerbox}

\q Why is gradient clipping needed for \textbf{long} rather than short sentences/texts?
\begin{answerbox}
BPTT multiplies gradients through every time step, so the contribution of an early step scales like (recurrent-Jacobian)$^{\,T}$ with $T=$ sequence length. If the spectral norm $>1$ this grows \textbf{exponentially in $T$} $\Rightarrow$ exploding gradients and unstable, NaN-prone updates. Short sequences have few factors, so the product stays bounded and clipping rarely triggers; only long sequences accumulate enough multiplications to blow up. Clipping caps the gradient norm, keeping the update direction but bounding its size.
\end{answerbox}

\q Why do Elman networks struggle to learn \textbf{long-term dependencies}?
\begin{answerbox}
\textbf{Vanishing gradients.} In BPTT the gradient w.r.t.\ a distant state is a product of many Jacobians $\prod W_{hh}^\top \mathrm{diag}(\phi')$. With a saturating activation (tanh, $\phi'\le 1$) and $\|W_{hh}\|<1$, this product shrinks exponentially with the time gap, so signal from far-back inputs reaches the update with negligible magnitude -- the net effectively can't tie a late output to an early input. (If $\|W_{hh}\|>1$ it instead explodes.) The single multiplicative hidden state has no path to carry information unchanged over many steps.
\end{answerbox}

\q In what situations would LSTMs, \emph{by design}, beat Elman networks?
\begin{answerbox}
Whenever the task needs \textbf{long-range memory}. The LSTM's cell state has \textbf{additive} updates gated by forget/input/output gates (a ``constant error carousel''), so gradients flow across many steps without vanishing, and the gates let it selectively keep or discard information. So LSTMs win on long sequences/documents, dependencies spanning many time steps, or when the relevant context appeared far in the past. For short sequences with only local dependencies, an Elman net does about as well with fewer parameters.
\end{answerbox}

\q How could you apply \textbf{conv layers} to a time-series forecasting task, and what must you watch out for?
\begin{answerbox}
Use \textbf{1D convolutions} sliding a kernel along the \emph{time} axis, with the feature/variable channels as input channels (multivariate $\to$ multiple $C_{\text{in}}$). Stack layers or use \textbf{dilated} convolutions (TCN/WaveNet) to enlarge the receptive field cheaply. Watch out for:
\begin{itemize}[leftmargin=*,nosep]
  \item \textbf{Causality}: use \emph{causal} (left-only) padding so the output at $t$ uses only $t$ and earlier -- ``same'' padding leaks future values (data leakage).
  \item \textbf{Receptive field} must cover the relevant past horizon (kernel size, depth, dilation).
  \item No leakage in the windowing / train--val--test split; scale-normalise inputs.
\end{itemize}
\end{answerbox}

\q Difference between a \texttt{Conv1d} with kernel size $w$ and a \texttt{Conv2d} with kernel size $(w,1)$?
\begin{answerbox}
\textbf{Numerically the same operation.} \texttt{Conv1d}$(w)$ on input $(N,C,L)$ slides a length-$w$ window along $L$ and sums over all $C$ channels (weights $C_{\text{out}}{\times}C_{\text{in}}{\times}w$). \texttt{Conv2d}$(w,1)$ on $(N,C,H,W)$ slides $w$ along $H$, $1$ (pointwise) along $W$ (weights $C_{\text{out}}{\times}C_{\text{in}}{\times}w{\times}1$). If you reshape the 1D signal to $(N,C,L,1)$, the two produce \emph{identical} results -- \texttt{Conv2d}$(w,1)$ is just \texttt{Conv1d}$(w)$ carrying a dummy singleton axis. Difference is only bookkeeping/overhead; prefer \texttt{Conv1d} for genuine 1D data. \textbf{Caution:} don't confuse with \texttt{Conv2d}$(w,\text{features})$, which treats features as a spatial axis instead of channels.
\end{answerbox}
...
\q In self-attention, what do the Query, Key and Value represent for each embedded token?
\begin{answerbox}
\textbf{Query (Q)}: the kind of information/features the current token is \emph{looking for}. \textbf{Key (K)}: the kind of information/features each token \emph{has to offer} (matched against queries). \textbf{Value (V)}: the actual information/features each token \emph{provides} (aggregated according to the attention weights). A query is compared to all keys to decide how much of each token's value to retrieve.
\end{answerbox}

\q What are the shapes of the projection matrices $W^Q, W^K, W^V$? Do their output dimensions have to be equal, and how do they compare to the embedding dimension?
\begin{answerbox}
With embedding dimension $d_{\text{model}}$, the projections map each token's embedding into query/key/value spaces:
\[
W^Q\in\mathbb{R}^{d_{\text{model}}\times d_k},\quad
W^K\in\mathbb{R}^{d_{\text{model}}\times d_k},\quad
W^V\in\mathbb{R}^{d_{\text{model}}\times d_v}.
\]
\textbf{Q and K must share the same dimension} $d_k$ -- the score $QK^\top$ is a dot product, which is only defined for equal-length vectors. \textbf{V may differ} ($d_v$): it is not dotted, only summed with the weights, so $d_v$ just sets the output size per head. \textbf{Vs the embedding}: $d_k, d_v$ are typically \emph{smaller}, usually $d_k=d_v=d_{\text{model}}/h$ for $h$ heads (e.g.\ $512/8=64$); concatenating the $h$ heads restores $d_{\text{model}}$. In the single-head case one often takes $d_k=d_v=d_{\text{model}}$.
\end{answerbox}
...
\q \textbf{Define supervised and unsupervised learning} and give typical applications of each.
\begin{answerbox}
\textbf{Supervised learning:} extract relevant features $x$ from raw observation data $o$ and \emph{learn a mapping from inputs $x$ to outputs $y$}, given a labelled training set of $(x,y)$ example pairs.
\emph{Applications:} recognition, planning, diagnosis, prediction.

\textbf{Unsupervised learning:} \emph{discover interesting structure} in the inputs $x$ given a training set -- no target outputs $y$ are provided.
\emph{Applications:} market segmentation, social network analysis, organizing computing clusters, astronomical data analysis.

\textbf{Key difference:} supervised has labels $y$ and learns $x\to y$; unsupervised has no labels and finds structure (e.g.\ clusters) in $x$ alone.
\end{answerbox}

\q \textbf{Supervised vs.\ unsupervised systems.} For each example, decide whether you would use a supervised or an unsupervised learning algorithm, and explain why.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Given email labeled as spam/not spam, learn a spam filter.
  \item Given a set of news articles found on the web, group them into sets of related articles.
  \item Given a database of customer data, automatically discover market segments and group customers into different segments.
  \item Given a dataset of patients diagnosed as either having glaucoma or not, learn to classify new patients as having glaucoma or not.
\end{enumerate}
\begin{answerbox}
The deciding factor is whether labels (target outputs) are given.
\textbf{(a) Supervised} -- emails carry spam/not-spam labels; learn a mapping input$\to$label (binary classification).
\textbf{(b) Unsupervised} -- no labels/categories given; we discover structure by clustering similar articles.
\textbf{(c) Unsupervised} -- segments are not predefined; clustering \emph{discovers} them from the data.
\textbf{(d) Supervised} -- each patient has a known glaucoma/no-glaucoma label; learn a classifier (binary classification).
\end{answerbox}

\q \textbf{Classification vs.\ regression systems.} Can we transform a regression problem into a classification problem? What would be the benefits of doing so?
\begin{answerbox}
Yes -- \textbf{discretize} the continuous target into bins/intervals, then predict the bin (e.g.\ house price $\to$ ``low/medium/high'', age $\to$ decade buckets). This is also called \emph{binning} or \emph{quantization}.

\textbf{Benefits:} handles non-linear / multi-modal targets without assuming a functional form; more robust to outliers and label noise; gives a calibrated probability per class and an easy notion of confidence; lets you use classification losses/metrics (cross-entropy, accuracy) and is often easier to optimize.
\textbf{Cost:} you lose precision/ordering information and must choose bin boundaries; too few bins underfit, too many give sparse, hard-to-learn classes.
\end{answerbox}

\q \textbf{Explain machine learning to your grandmother.} Define machine learning as simply as possible.
\begin{answerbox}
Instead of writing down exact rules for a computer to follow, we \textbf{show it many examples} and let it \emph{figure out the rules by itself}. Like a child learning what a ``cat'' is by seeing lots of cats rather than reading a definition -- the machine learns patterns from experience and gets better with more examples.

\textbf{Textbook version (Mitchell):} a program learns from experience $E$ w.r.t.\ task $T$ and performance measure $P$ if its performance at $T$, measured by $P$, improves with $E$.
\end{answerbox}

\q \textbf{Why do we need such systems?} Why use machine learning instead of hand-written rules?
\begin{answerbox}
For many problems, \textbf{explicit rules are impossible or impractical} to write by hand:
\textbf{Complexity} -- tasks like image/speech recognition have too many cases and exceptions to enumerate.
\textbf{Scale} -- ML automates decisions on huge data volumes that humans can't process.
\textbf{Adaptivity} -- models adapt to new/changing data without being reprogrammed.
\textbf{Discovery} -- they reveal patterns/correlations humans wouldn't spot.
\textbf{Personalization} -- tailor outputs per user (recommendations, filters).
\end{answerbox}

\q \textbf{Difference between ML and DL.}
\begin{answerbox}
DL is a \textbf{subset} of ML using deep (many-layered) neural networks.
\textbf{Features:} classical ML needs \emph{hand-crafted} features + a model; DL \emph{learns} features automatically (end-to-end) from raw data.
\textbf{Data/compute:} DL needs much more data and compute (GPUs); classical ML works on smaller datasets.
\textbf{Scaling:} DL performance keeps improving with more data, classical ML plateaus.
\textbf{Interpretability:} classical ML is often more interpretable; DL is more of a ``black box''.
\end{answerbox}

\q \textbf{Dangers of DL.} What are the risks/limitations of deep learning?
\begin{answerbox}
\textbf{Black box} -- hard to interpret/explain decisions (problem in medicine, law, finance).
\textbf{Bias} -- learns and amplifies biases present in the training data (unfair/discriminatory outputs).
\textbf{Data hunger} -- needs large labeled datasets; expensive and may not exist.
\textbf{Overconfidence \& brittleness} -- can be confidently wrong and fooled by \emph{adversarial examples}; fails on out-of-distribution data.
\textbf{Cost \& environment} -- heavy compute, high energy consumption.
\textbf{Misuse} -- deepfakes, surveillance, disinformation; over-reliance/automation without accountability.
\end{answerbox}
...
\q Define the rank and shape of a \textbf{grayscale image} that is $900$ wide and $550$ tall.
\begin{answerbox}
Rank 2. Shape \texttt{(550, 900)} following (height, width); a single intensity value per pixel means no channel axis. Frameworks often keep an explicit channel dimension though: \textbf{PyTorch} stores it channels-first as \texttt{(1, 550, 900)} -- i.e.\ rank 3 with $C=1$ (\texttt{ToTensor} always returns \texttt{(C, H, W)}); PIL/NumPy/TensorFlow use channels-last \texttt{(550, 900, 1)}.
\end{answerbox}

\q Define the rank and shape of a \textbf{color image} that is $320$ wide and $480$ tall.
\begin{answerbox}
Rank 3. \textbf{NumPy} (channels-last, the reference here): \texttt{(480, 320, 3)} as (height, width, channels) with 3 RGB channels -- this is what \texttt{imageio}/PIL$\to$\texttt{np.array} give. PyTorch reorders to channels-first \texttt{(3, 480, 320)}.
\end{answerbox}

\q Define the rank and shape of a \textbf{simple 2-D embedding} of a single word, e.g.\ $\mathrm{beer}=[-0.9,-1.0]$.
\begin{answerbox}
A single embedding vector is rank 1 with shape \texttt{(2,)} (one number per feature axis: solidity, eatability). A table of $V$ such words would be rank 2, shape \texttt{(V, 2)}. \textbf{NumPy:} \texttt{np.array([-0.9, -1.0])} has shape \texttt{(2,)}; stacking the vocabulary gives \texttt{(V, 2)}.
\end{answerbox}

\q Define the rank and shape of \textbf{stereo sound}, $2$ seconds at a sampling rate of $44.1$\,kHz.
\begin{answerbox}
Samples per channel $= 2 \times 44100 = 88200$, with 2 channels (left/right). Rank 2. \textbf{NumPy:} \texttt{scipy.io.wavfile.read} returns \texttt{(88200, 2)} as (samples, channels). Audio libraries that put channels first store \texttt{(2, 88200)}.
\end{answerbox}

\q For the \textbf{MNIST} dataset ($60000$ images, $28\times28$ grayscale pixels), give the rank and shape of: (a) one image, (b) the whole dataset, (c) a batch of $32$ images.
\begin{answerbox}
\textbf{NumPy} (channels-last reference): (a) one image rank 2, \texttt{(28, 28)}; (b) whole dataset rank 3, \texttt{(60000, 28, 28)}; (c) batch of 32 rank 3, \texttt{(32, 28, 28)}. Adding an explicit channel axis (grayscale $=1$) raises each by one rank: NumPy/TensorFlow channels-last \texttt{(32, 28, 28, 1)}, PyTorch channels-first \texttt{(32, 1, 28, 28)}.
\end{answerbox}

\q \textbf{What would be a 5-D tensor?} Give a concrete example and break down the rank.
\begin{answerbox}
A \textbf{batch of colored videos}. A colored frame is rank 3 (height, width, channels) $+$ time/frames (rank 1) $+$ batch (rank 1) $=$ rank 5. \textbf{NumPy} (channels-last): \texttt{(batch, frames, height, width, channels)}; PyTorch channels-first: \texttt{(batch, frames, channels, height, width)}.
\end{answerbox}
...
\q \textbf{Write down the algorithm of the Gradient Descent method in pseudo code.}
\begin{answerbox}
Goal: minimise cost function $C(\theta)$ w.r.t.\ the model parameters $\theta$.
\begin{tabbing}
xx\=xx\=xx\=\kill
\textbf{1.}\> Initialise parameters $\theta^{(0)}$ (e.g.\ random); set $t \leftarrow 0$ \\[2pt]
\textbf{2.}\> \textbf{repeat} (update step $t \to t+1$): \\
\> \textbf{(i)} compute gradient at current position: $g \leftarrow \nabla C(\theta^{(t)})$ \\
\> \textbf{(ii)} step in negative-gradient direction: $\theta^{(t+1)} \leftarrow \theta^{(t)} - \alpha\, \nabla C(\theta^{(t)})$ \\
\> \>\quad $t \leftarrow t+1$ \\[2pt]
\textbf{3.}\> \textbf{until} the change in $\theta$ is very small ($\lVert \theta^{(t+1)}-\theta^{(t)} \rVert < \varepsilon$)
\end{tabbing}
\textbf{Notation:} $\alpha$ -- learning rate (step size); $\nabla C(\theta^{(t)})$ -- gradient of the cost at the last position reached.
\textbf{Key fact:} $\theta^{(t)}$ and $\nabla C(\theta^{(t)})$ have the \emph{same dimensionality}, so the update is element-wise per parameter.
\end{answerbox}

\q \textbf{Short conceptual questions on optimisation and generalisation.}

\textbf{(a) In what sense are optimisation techniques important for ML?}
\begin{answerbox}
Training a model \emph{is} an optimisation problem: we minimise a cost $J(\theta)$ over the parameters $\theta$. Without efficient optimisation (gradient descent and its variants) we could not fit models with millions of parameters. The chosen optimiser determines whether training converges, how fast, and to how good a solution -- so optimisation directly controls feasibility, training time, and final accuracy.
\end{answerbox}

\textbf{(b) Which problems can gradient descent be applied to? When is the solution unique? What can go wrong in general?}
\begin{answerbox}
\textbf{Applicable to:} any problem with a \emph{differentiable} cost where we can compute (or estimate) the gradient $\nabla C(\theta)$.
\textbf{Unique solution:} when the cost is \emph{convex} (e.g.\ a single global minimum, as in linear regression / MSE or logistic regression) -- GD converges to the global optimum regardless of initialisation (with a suitable learning rate).
\textbf{General (non-convex, e.g.\ deep nets):} the surface has many local minima, saddle points, and flat/plateau regions, so GD may get stuck in a poor local minimum or saddle, converge slowly, and the result \emph{depends on initialisation}. It also requires differentiability and a well-chosen learning rate.
\end{answerbox}

\textbf{(c) Why is MSE cost less suitable for classification?}
\begin{answerbox}
With sigmoid/softmax outputs, MSE gives a \textbf{non-convex} loss and \emph{small/vanishing gradients} when the output is confidently wrong (the sigmoid saturates), so learning stalls. Cross-entropy is convex in the logits and yields a clean gradient $\hat y - y$ that is large exactly when the prediction is badly wrong -- faster, more reliable learning. MSE also assumes Gaussian errors, which does not match discrete class labels.
\end{answerbox}

\textbf{(d) What may happen if the learning rate is too large?}
\begin{answerbox}
The updates overshoot the minimum; the loss \textbf{oscillates, fails to converge, or diverges} (blows up to NaN/$\infty$). Effectively you jump back and forth across (or out of) the valley instead of descending into it.
\end{answerbox}

\textbf{(e) Why can the test error start increasing after some epochs?}
\begin{answerbox}
\textbf{Overfitting.} After a point the model stops learning general patterns and starts memorising noise/idiosyncrasies of the training set: training error keeps falling while test (generalisation) error rises. This is the classic U-shaped test-error curve and motivates \emph{early stopping}.
\end{answerbox}

\textbf{(f) With more training data, does reaching a small \emph{training} cost get easier or harder?}
\begin{answerbox}
\textbf{Harder.} More (and more varied) examples are more difficult to fit \emph{all} at once, so the achievable training cost is typically higher and the train/test gap shrinks. Memorising a tiny dataset is easy (low training cost, poor generalisation); fitting a large one forces the model to learn genuine structure -- worse training cost but better generalisation.
\end{answerbox}
...
\q \textbf{Bias/variance, split ratio, double descent, and regularisation.}

\textbf{(a) Explain bias error and variance error. What makes each larger/smaller?}
\begin{answerbox}
\textbf{Bias error:} error from wrong/too-simple assumptions -- the model cannot capture the true relationship (\emph{underfitting}). Large when the model is too simple (too few parameters/layers, too much regularisation, wrong features). Reduced by increasing capacity, adding features, training longer, less regularisation.

\textbf{Variance error:} error from sensitivity to the particular training set -- the model fits noise and changes a lot with different data (\emph{overfitting}). Large when the model is too complex relative to the data, or the dataset is small. Reduced by more training data, regularisation (L1/L2, dropout), data augmentation, a simpler model, or ensembling/early stopping.
\end{answerbox}

\textbf{(b) Why does training error increase, and validation error decrease, with the split ratio (fraction used for training)?}
\begin{answerbox}
\textbf{Training error $\uparrow$:} with a larger split ratio there are \emph{more} training samples to fit simultaneously, which is harder -- a small set is easy to memorise (low train error), a large diverse set forces compromises (higher train error).

\textbf{Validation error $\downarrow$:} more training data lets the model learn the true underlying structure and generalise better, so it performs better on the held-out validation set. (At the same time the validation set shrinks, but the dominant effect is better generalisation from more training data.)
\end{answerbox}

\textbf{(c) What is the "deep double descent" phenomenon?}
\begin{answerbox}
As model size (or training time/data) grows, test error first follows the classical U-curve -- decreasing, then \emph{increasing} as we approach the \textbf{interpolation threshold} (just enough capacity to fit the training data exactly). Beyond that point, in the \emph{over-parameterised} regime, the test error \textbf{decreases again} -- a second descent. So very large models can generalise well despite fitting the training data perfectly, contradicting the classical bias--variance picture.
\end{answerbox}

\textbf{(d) What is the goal of regularisation in deep learning?}
\begin{answerbox}
To \textbf{reduce overfitting / variance and improve generalisation} to unseen data -- by constraining the model's effective capacity or adding prior information (e.g.\ a preference for small/simple weights), so it learns robust patterns instead of memorising training noise. It trades a little more bias for much less variance.
\end{answerbox}

\textbf{(e) List four regularisation methods and describe each in two sentences.}
\begin{answerbox}
\textbf{L2 (weight decay):} adds $\lambda\lVert\theta\rVert_2^2$ to the loss, penalising large weights. This shrinks all weights smoothly toward zero, giving a smoother, less complex function.

\textbf{L1:} adds $\lambda\lVert\theta\rVert_1$ to the loss. It drives many weights to exactly zero, producing sparse models and implicit feature selection.

\textbf{Dropout:} randomly zeros a fraction $p$ of activations each training step. This prevents co-adaptation of neurons and acts like training an ensemble of sub-networks, improving robustness.

\textbf{Early stopping:} monitor validation error during training and stop when it starts rising. This halts before the model overfits the training data, effectively limiting capacity. (Alternatives: \emph{data augmentation} -- enlarge/vary the dataset with label-preserving transforms; \emph{batch normalisation} -- adds noise/scaling that regularises.)
\end{answerbox}

\q \textbf{Reading a learning curve (activity).} An image classifier (MobileNetV2) is trained for 30 epochs. \emph{Training} accuracy rises steadily from $\sim$0.75 to $\sim$0.88, while \emph{validation} accuracy rises to $\sim$0.82 around epoch 15--20 and then \textbf{plateaus and slightly declines} (to $\sim$0.79). The gap between the two curves keeps widening.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item What can we say about this evolution of accuracy along the epochs?
  \item What can we do with an ``improvement budget'' (engineering time) of: 0h more, 15h more, 150h more?
\end{enumerate}
\begin{answerbox}
\textbf{(a) Diagnosis -- overfitting.} Training accuracy keeps improving while validation accuracy stalls (\textasciitilde epoch 15--20) and then drops, so the train/val gap widens: the model is fitting training-set noise and generalising worse. The best generalising model is \emph{not} the last epoch but the one at peak validation accuracy.

\textbf{(b) What to do per budget:}
\textbf{0h more (free, no training):} use \emph{early stopping} -- pick the checkpoint at peak validation accuracy ($\sim$epoch 15--20) instead of the final model. No extra work, immediate gain.

\textbf{15h more (light effort):} add \emph{regularisation / data augmentation} and re-tune -- dropout, weight decay, image augmentation (flips, crops, colour jitter), tune the learning rate / LR schedule. Cheap changes that directly attack the overfitting.

\textbf{150h more (large effort):} attack the data and architecture -- \emph{collect/label more data}, try a larger or different backbone, transfer learning / fine-tuning, hyper-parameter search, ensembling. Highest potential gain but expensive.
\end{answerbox}
...
\q \textbf{CNN fundamentals.}

\textbf{(a) What does it mean to ``preserve the spatial structure'' in a CNN?}
\begin{answerbox}
The input keeps its 2D (or 3D) grid layout throughout the conv/pool layers instead of being flattened into a 1D vector (as an MLP would). Neighbouring pixels stay neighbours, so the network can exploit \emph{local spatial relationships} (edges, textures, shapes) and translation structure -- which would be destroyed by flattening.
\end{answerbox}

\textbf{(b) Explain the operations performed when convolving a filter over an input.}
\begin{answerbox}
The filter (small weight kernel) \emph{slides} over the input. At each position it performs an \textbf{element-wise multiplication} of the filter weights with the overlapping input patch, then \textbf{sums} the products (a dot product), and adds a \textbf{bias} -- giving one scalar in the output. Sliding over all positions (controlled by stride/padding) produces the 2D \emph{activation map}; an activation function (e.g.\ ReLU) is then applied element-wise.
\end{answerbox}

\textbf{(c) Shape of the parameter tensor of one filter in a Conv3D layer? Tunable hyper-parameters?}
\begin{answerbox}
One Conv3D filter spans all input channels, so its weight tensor has shape $(C_{in}, D, H, W)$ -- depth $\times$ height $\times$ width $\times$ input channels -- plus \textbf{1 bias} scalar. (E.g.\ a $3\times3\times3$ filter over $C_{in}$ channels $= 27\,C_{in}+1$ params.)
\textbf{Tunable hyper-parameters:} kernel size, number of filters (= output channels), stride, padding, dilation. (The weight \emph{values} are learned, not tuned.)
\end{answerbox}

\textbf{(d) Define receptive field and activation map.}
\begin{answerbox}
\textbf{Receptive field:} the region of the input that influences (is ``seen by'') a particular output neuron. It grows with depth as layers stack.
\textbf{Activation map (feature map):} the 2D output produced by applying one filter across the whole input -- it shows where that filter's pattern is detected.
\end{answerbox}

\textbf{(e) Explain sparse connectivity and parameter sharing.}
\begin{answerbox}
\textbf{Sparse connectivity (local connectivity):} each output neuron connects only to a small local patch of the input (the kernel region), not to every input -- far fewer connections than a fully-connected layer.
\textbf{Parameter sharing:} the \emph{same} filter weights are reused at every spatial location. This gives translation equivariance and drastically reduces the number of parameters.
\end{answerbox}

\textbf{(f) Effect of stride and zero-padding.}
\begin{answerbox}
\textbf{Stride $s$:} step size of the filter. Larger stride $\Rightarrow$ filter skips positions $\Rightarrow$ \emph{smaller} output (downsampling), less computation.
\textbf{Zero-padding $p$:} adds zeros around the border. It lets the filter cover edge pixels and \emph{controls output size} -- ``same'' padding keeps output size equal to input; without padding the output shrinks each layer.
Output size: $O = \left\lfloor \dfrac{I - K + 2p}{s} \right\rfloor + 1$.
\end{answerbox}

\textbf{(g) Compute the number of parameters of a convolution layer.}
\begin{answerbox}
For a 2D conv layer with kernel $K\times K$, $C_{in}$ input channels, $C_{out}$ filters (with bias):
\[ \#\text{params} = (K \cdot K \cdot C_{in} + 1)\cdot C_{out}. \]
The ``$+1$'' is the per-filter bias. \emph{Example:} $5\times5$, $C_{in}=3$, $C_{out}=10$ $\Rightarrow (5\cdot5\cdot3+1)\cdot10 = 760$. Note this is independent of the input spatial size (parameter sharing).
\end{answerbox}

\textbf{(h) What is a max pooling layer and its role?}
\begin{answerbox}
A layer that slides a window over each activation map and outputs the \textbf{maximum} value in each window. It \emph{downsamples} (reduces spatial size and computation), keeps the strongest/most salient activations, and adds a degree of \emph{translation invariance}. It has \textbf{no learnable parameters}.
\end{answerbox}

\textbf{(i) Can simple CNN architectures (as in this PW) address object detection or segmentation?}
\begin{answerbox}
\textbf{No -- not directly.} A plain CNN classifier outputs a single label for the whole image; it does not localise objects (detection $=$ class $+$ bounding box) or label every pixel (segmentation). These tasks need specialised architectures: detection $\to$ R-CNN/YOLO/SSD; segmentation $\to$ FCN/U-Net/Mask R-CNN, which build on CNN backbones but add localisation/up-sampling heads.
\end{answerbox}
...
\q \textbf{(a)} How can you define a generative system? Describe the two approaches seen in class to build generative systems with RNNs.
\begin{answerbox}
A \emph{generative system} learns the data distribution $p(x)$ (or $p(x\mid c)$) so it can \emph{sample new data} that resembles the training set, rather than only predicting a label $p(y\mid x)$ as a discriminative model does.

Two RNN-based approaches:
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Character/token-level autoregressive RNN.} Train the RNN to predict the next symbol given the previous ones, modelling $p(x_t\mid x_{1:t-1})$ via softmax. To generate: feed a seed, sample the next token, feed it back as input, and repeat (teacher forcing at train time, feedback loop at generation time).
\item \textbf{Sequence-to-sequence (encoder--decoder) RNN.} An encoder RNN compresses an input/condition into a context vector; a decoder RNN generates the output sequence autoregressively from it. This produces \emph{conditional} samples (e.g.\ translation, image captioning).
\end{itemize}
\end{answerbox}

\q \textbf{(b)} How can you define a generative system for sequence data? Describe the two approaches seen in class to build generative systems for sequence data.
\begin{answerbox}
A generative system for sequence data models the joint distribution of a sequence, factorised autoregressively as $p(x_{1:T})=\prod_t p(x_t\mid x_{1:t-1})$, so that new sequences can be sampled one element at a time.

Two approaches:
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Unconditional autoregressive generation:} a single RNN predicts the next element; sampling and feeding back generates a free-running sequence (e.g.\ text/music generation from a seed).
\item \textbf{Conditional generation with Seq2Seq:} an encoder turns an input sequence into a context, and a decoder generates the target sequence conditioned on it (e.g.\ machine translation, summarisation).
\end{itemize}
\textit{(Same two ideas as in (a), now stated explicitly for the sequence case.)}
\end{answerbox}

\q \textbf{(c)} Give two desired properties of word embeddings when used in a classification task (e.g.\ sentiment classification).
\begin{answerbox}
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Semantic similarity / generalisation:} words with similar meaning have nearby vectors (small cosine distance), so the classifier generalises to words unseen with a given label.
\item \textbf{Low, dense, fixed dimensionality:} a compact dense vector (vs.\ sparse one-hot) reduces parameters and overfitting and captures shared features.
\end{itemize}
\textit{(Also acceptable: task-discriminative / linearly separable for the labels, and capturing relevant relations such as polarity.)}
\end{answerbox}

\q \textbf{(d)} What are the three strategies to use an embedding layer in a deep system?
\begin{answerbox}
\begin{enumerate}[leftmargin=*,nosep,label=\arabic*.]
\item \textbf{Train from scratch:} randomly initialise the embedding layer and learn it jointly with the task (good with a large labelled dataset).
\item \textbf{Use pretrained, frozen:} load pretrained vectors (Word2Vec/GloVe/FastText) and keep them fixed -- only the rest of the network trains (good with little data).
\item \textbf{Use pretrained, fine-tuned:} initialise with pretrained vectors but let them update during training (best when you have enough data to adapt them to the task).
\end{enumerate}
\end{answerbox}

\q \textbf{(f)} What is the difference between Word2Vec and FastText?
\begin{answerbox}
Word2Vec treats each word as an \emph{atomic} unit -- one vector per word in the vocabulary -- so it cannot represent out-of-vocabulary words and ignores morphology. FastText represents a word as the \emph{sum of its character $n$-gram (subword) embeddings}, so it shares parameters across morphologically related words and can build vectors for unseen/rare words and misspellings. FastText is better for morphologically rich languages; Word2Vec is lighter.
\end{answerbox}

\q \textbf{(g)} \textit{Optional (ex.\ 3).} Draw the architecture of a Seq2Seq model and explain the encoder/decoder concept.
\begin{answerbox}
\textbf{Encoder:} an RNN (often LSTM/GRU) reads the input sequence and compresses it into a fixed-size \emph{context vector} (its final hidden state). \textbf{Decoder:} a second RNN is initialised with that context and generates the output sequence one token at a time, feeding each predicted token back as the next input until \texttt{<eos>}.

\begin{center}
\begin{tikzpicture}[
    node distance=7mm and 8mm,
    enc/.style={draw,rounded corners,minimum height=8mm,minimum width=8mm,fill=blue!8},
    dec/.style={draw,rounded corners,minimum height=8mm,minimum width=8mm,fill=orange!12},
...
% context
\node[draw,ellipse,fill=green!12,right=of e3] (c) {context};
% decoder cells
\node[dec,right=of c] (d1) {$s_1$};
\node[dec,right=of d1] (d2) {$s_2$};
\node[dec,right=of d2] (d3) {$s_3$};
% inputs
\node[below=of e1] (x1) {$x_1$};
\node[below=of e2] (x2) {$x_2$};
\node[below=of e3] (x3) {$\langle\text{eos}\rangle$};
% outputs
\node[above=of d1] (y1) {$y_1$};
\node[above=of d2] (y2) {$y_2$};
\node[above=of d3] (y3) {$\langle\text{eos}\rangle$};
% edges
\draw[->] (x1)--(e1); \draw[->] (x2)--(e2); \draw[->] (x3)--(e3);
\draw[->] (e1)--(e2); \draw[->] (e2)--(e3); \draw[->] (e3)--(c);
\draw[->] (c)--(d1); \draw[->] (d1)--(d2); \draw[->] (d2)--(d3);
\draw[->] (d1)--(y1); \draw[->] (d2)--(y2); \draw[->] (d3)--(y3);
% feedback of generated tokens
\draw[->,dashed] (y1) to[bend left=35] (d2);
\draw[->,dashed] (y2) to[bend left=35] (d3);
\node[below=2mm of e2,font=\scriptsize\itshape] {encoder};
\node[below=2mm of d2,font=\scriptsize\itshape] {decoder};
\end{tikzpicture}
\end{center}
\textit{Dashed arrows = the previous output token fed back as the decoder's next input.}
\end{answerbox}

\q \textbf{(h)} \textit{Optional (ex.\ 3).} What is the main problem of a basic Seq2Seq model used for machine translation, and the solution to overcome it?
\begin{answerbox}
\textbf{Problem -- the fixed-size context bottleneck:} the whole source sentence must be squeezed into one context vector. For long sentences this loses information and translation quality degrades sharply.

\textbf{Solution -- attention:} instead of a single context, the decoder computes at \emph{each} output step a weighted combination of \emph{all} encoder hidden states, focusing on the relevant source words. This removes the bottleneck, handles long sequences, and yields soft alignments between source and target words.
\end{answerbox}
...
\q An autoencoder is used as a lossy \emph{compression} tool (encoder $\to$ small latent code $\to$ decoder). How could we evaluate its performance, both in terms of loss and in terms of gain of bandwidth?
\begin{answerbox}
\textbf{Two axes -- they trade off against each other.}
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Loss / reconstruction quality} (how much information is lost):
  reconstruction error on held-out data, e.g.\ MSE $\frac{1}{N}\sum\|x-\hat{x}\|^2$,
  PSNR $=10\log_{10}(\mathrm{MAX}^2/\mathrm{MSE})$ or SSIM for images. Lower MSE / higher PSNR $=$ better fidelity.
  \item \textbf{Gain of bandwidth / compression ratio} (how much we save):
  $\text{CR}=\dfrac{\text{size of }x}{\text{size of latent code}}=\dfrac{\dim(x)}{\dim(z)}$
  (or bits-original\,/\,bits-code if the code is quantised/entropy-coded). Higher $=$ more saving.
\end{itemize}
Report the pair together as a \textbf{rate--distortion curve}: sweep the latent size $\dim(z)$ and plot reconstruction error (distortion) vs.\ compression ratio (rate). The best model dominates -- lower distortion at the same rate. A single number hides the trade-off: a tiny code gives huge bandwidth gain but high loss.
\end{answerbox}

\q After training that autoencoder, are you now able to use the encoded features to train a simple SVM classifier?
\begin{answerbox}
Yes. The encoder maps each input to a compact latent vector $z=\mathrm{enc}(x)$; these $z$ are dense, low-dimensional \textbf{features} that can directly feed any downstream classifier. Procedure: (1) train the autoencoder unsupervised on (possibly unlabelled) data; (2) freeze the encoder and run the \emph{labelled} samples through it to get $\{(z_i,y_i)\}$; (3) train the SVM on $(z_i,y_i)$. This is the classic \emph{unsupervised pretraining / feature-extraction} use of an autoencoder: the bottleneck removes redundancy, so the SVM works in a smaller, better-conditioned space and needs fewer labels than training on raw $x$.
\end{answerbox}

\q (a) Why (or when) do we need the functional API of Keras?
\begin{answerbox}
The \texttt{Sequential} API only builds a plain stack: one input, one output, layers in a line. The \textbf{functional API} treats layers as callable functions on tensors, so it can express \emph{non-sequential} graphs: multiple inputs or outputs, branches that merge (concatenate/add), skip/residual connections, shared layers, and arbitrary DAGs (e.g.\ Inception, U-Net, siamese, multi-task heads). Use it whenever the topology is not a simple chain.
\end{answerbox}

\q (b) What are the benefits of using transfer learning?
\begin{answerbox}
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Less data needed} -- reuse features learned on a large source dataset, so the target task needs far fewer labelled samples.
  \item \textbf{Faster training / less compute} -- start from good weights instead of random init; often only train a small head.
  \item \textbf{Better generalisation} -- pretrained low-level features (edges, textures) are general and reduce overfitting on small targets.
  \item \textbf{Higher final accuracy} when the source and target domains are related.
\end{itemize}
\end{answerbox}

\q (c) What does \emph{freezing} mean in a transfer-learning strategy? What is the influence of the training-set size on freezing?
\begin{answerbox}
\textbf{Freezing} $=$ setting layers as non-trainable (\texttt{layer.trainable=False}): their weights are kept fixed during training, so gradients don't update them and they act as a fixed feature extractor. Influence of dataset size:
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Small target set} $\Rightarrow$ freeze \emph{most/all} of the backbone, train only the new head. Few parameters to fit $\Rightarrow$ less overfitting.
  \item \textbf{Large target set} $\Rightarrow$ freeze \emph{little or nothing}; unfreeze (fine-tune) more layers with a small learning rate, because there is enough data to adapt the deeper, task-specific features safely.
\end{itemize}
Rule of thumb: the more (and the more domain-different) your data, the fewer layers you freeze.
\end{answerbox}

\q (d) What are the potential usages of auto-encoders?
\begin{answerbox}
Dimensionality reduction / compression; denoising (denoising AE); anomaly / outlier detection (high reconstruction error $=$ anomaly); unsupervised feature extraction \& pretraining; data generation (variational AE); missing-value imputation; representation learning / visualisation of the latent space.
\end{answerbox}

\q (e) In which situations would you use auto-encoders to extract features?
\begin{answerbox}
When you have \textbf{plenty of unlabelled data but few labels}, and want a compact unsupervised representation; when raw inputs are high-dimensional / redundant and you need denoising or dimensionality reduction before a downstream model; when there is \textbf{no suitable pretrained network} for your domain (e.g.\ sensor signals, medical/spectral data) so off-the-shelf transfer learning doesn't apply.
\end{answerbox}

\q (f) What are the principles of self-supervised learning?
\begin{answerbox}
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Labels come from the data itself} -- no human annotation. A \emph{pretext task} is defined by hiding/transforming part of the data and asking the model to predict it (mask a word, predict rotation, fill a patch, contrast two views).
  \item Solving the pretext task forces the encoder to learn \textbf{general, semantic representations}.
  \item These representations are then \textbf{transferred} to the real (downstream) task, fine-tuned with the few available labels.
\end{itemize}
It is the unlabelled-data way to get the benefits of pretraining (basis of BERT, GPT, SimCLR).
\end{answerbox}

\q (g) When would you prefer auto-encoders or self-supervised learning instead of using pre-trained networks to extract features?
\begin{answerbox}
Use AE/SSL (train your own representation on your own unlabelled data) when:
\begin{itemize}[leftmargin=1.2em]
  \item Your \textbf{domain is far from the pretrained model's} source data (ImageNet $\neq$ ECG, radar, audio spectrograms, text in a niche language) -- transferred features wouldn't be relevant.
  \item There is \textbf{no good pretrained model} for your modality/architecture.
  \item You have \textbf{lots of unlabelled in-domain data} to exploit, but few labels.
  \item You need a representation \emph{tailored} to your data distribution (e.g.\ anomaly detection).
\end{itemize}
Conversely, prefer a pre-trained network when a model trained on a large, \emph{similar} domain is available -- it is cheaper and usually stronger than training from scratch.
\end{answerbox}
...
\q Define a Seq2Seq model. What is special about the kind of mapping it learns?
\begin{answerbox}
A model that maps an input sequence to an output sequence of \emph{possibly different length}, where the \textbf{entire input must be read before} the target can start being predicted. Typical uses: machine translation, question-answering bots, grapheme-to-phoneme mapping.
\end{answerbox}

\q Describe the original RNN-based Seq2Seq architecture and what is passed between its two parts.
\begin{answerbox}
Two RNNs: an \textbf{encoder} reads the whole input and compresses it into a fixed-size \textbf{context vector} (its final hidden state $h_t$); a \textbf{decoder} is initialised with that context vector and generates the output token by token. In the original form, \emph{only the encoder's last state} is passed as the decoder's initial state.
\end{answerbox}

\q What other names are given to the context vector, and what is it supposed to represent?
\begin{answerbox}
A numerical summary of the meaning of the whole input sequence. When the inputs are words it is called a \textbf{sentence embedding} or \textbf{thought vector}.
\end{answerbox}

\q What are the \texttt{<BOS>} and \texttt{<EOS>} tokens for in a Seq2Seq decoder?
\begin{answerbox}
\texttt{<BOS>} (beginning of sentence) is the first input that kicks off decoding; the decoder generates autoregressively (feeding $y_{j-1}$ to predict $y_j$) until it emits \texttt{<EOS>} (end of sentence), which stops generation.
\end{answerbox}

\q In a Seq2Seq model, why can decoding not start before the whole input has been read?
\begin{answerbox}
The decoder is initialised from the encoder's context vector, which summarises the \emph{entire} source. Output tokens can depend on any part of the input (e.g.\ word order differs across languages), so a partial summary would be insufficient.
\end{answerbox}

\q Distinguish global from local attention.
\begin{answerbox}
\textbf{Global} attention uses \emph{all} encoder hidden states when forming the context; \textbf{local} attention uses only a \emph{subset} of them.
\end{answerbox}

\q List the steps that turn encoder states + the current decoder state into a context vector.
\begin{answerbox}
(1) \textbf{Score} each encoder state against the decoder state with an alignment/score function (e.g.\ dot product); (2) \textbf{softmax} the scores into an attention distribution (sums to 1); (3) \textbf{multiply} each encoder state by its weight (alignment vectors); (4) \textbf{sum} the alignment vectors into the context vector; (5) \textbf{feed} it to the decoder (Bahdanau 2015: concatenate with the hidden state).
\end{answerbox}

\q What does ``self'' mean in self-attention, and how does it differ from Seq2Seq attention?
\begin{answerbox}
Same \emph{objective} (an enhanced, context-aware representation), but self-attention looks only at inputs from the \emph{same sequence} -- not at states coming from another part of the model (e.g.\ the encoder). Hence ``self''.
\end{answerbox}

\q How are Q, K and V obtained, and what is a common misconception about them?
\begin{answerbox}
By projecting each input embedding with three learned matrices: $q_i=x_iW^Q$, $k_i=x_iW^K$, $v_i=x_iW^V$ (like three Dense layers without bias, weights shared across positions). Misconception: they are \emph{not} three different inputs -- in self-attention they are three projections of the \emph{same} input.
\end{answerbox}

\q What is the computational complexity of self-attention in the sequence length $n$, and why?
\begin{answerbox}
$O(n^2)$: every token attends to every other token, so the attention matrix has $n\times n$ entries. This motivates efficient/long-context variants.
\end{answerbox}

\q Compare the inner structure of a Transformer encoder module and a decoder module.
\begin{answerbox}
\textbf{Encoder}: self-attention + feed-forward. \textbf{Decoder}: \emph{masked} self-attention + encoder--decoder (cross) attention + feed-forward. The cross-attention layer plays the role of attention in classic Seq2Seq.
\end{answerbox}

\q Contrast encoder-only and decoder-only Transformer models.
\begin{answerbox}
\textbf{Encoder-only} (e.g.\ BERT): bidirectional, good for understanding/representation tasks. \textbf{Decoder-only} (e.g.\ GPT): causal/autoregressive (masked self-attention), good for generation. Pre-trained sub-parts can be used alone.
\end{answerbox}

\q Why is multi-head attention used instead of a single attention function?
\begin{answerbox}
Several attention heads run in parallel and are concatenated, letting the model attend to different relations/subspaces simultaneously (loosely analogous to multiple filters in a CNN).
\end{answerbox}
...
\q In a diffusion model, which process is fixed and which is learned?
\begin{answerbox}
The \textbf{forward} process $q(x_t|x_{t-1})$ is \textbf{fixed} (no parameters) -- it just adds Gaussian noise according to the schedule. The \textbf{reverse} process $p_\theta(x_{t-1}|x_t)$ is \textbf{learned} by a neural network -- it denoises step by step. Trap: swapping the two.
\end{answerbox}

\q Write a single forward (noising) step of a diffusion model.
\begin{answerbox}
$x_t=\sqrt{1-\beta_t}\,x_{t-1}+\sqrt{\beta_t}\,\epsilon$ with $\epsilon\sim\mathcal{N}(0,\mathbf{1}_d)$, i.e.\ the Gaussian kernel $q(x_t|x_{t-1})=\mathcal{N}(x_t;\sqrt{1-\beta_t}\,x_{t-1},\beta_t\mathbf{1}_d)$. Here $\beta_t$ is the variance schedule.
\end{answerbox}

\q Why do we choose a small $\beta_t$ and a large number of steps $T\approx1000$?
\begin{answerbox}
Small per-step noise keeps each reverse step approximately \textbf{Gaussian}, so a network can model it; using many steps still lets $x_T$ reach (essentially) \textbf{pure noise} $\mathcal{N}(0,I)$.
\end{answerbox}

\q What does the diffusion network actually predict, and what loss is used?
\begin{answerbox}
It predicts the \textbf{noise} $\epsilon$ added at step $t$: $\epsilon_\theta(x_t,t)$. Trained with the simple $\ell_2$ regression $L_{\text{simple}}=\mathbb{E}\big[\|\epsilon-\epsilon_\theta(\sqrt{\bar\alpha_t}\,x_0+\sqrt{1-\bar\alpha_t}\,\epsilon,\,t)\|^2\big]$.
\end{answerbox}

\q Why can't we train a diffusion model by directly maximising the likelihood, and what is done instead?
\begin{answerbox}
The negative log-likelihood $-\sum_{x_0}\log p_\theta(x_0)$ is \textbf{intractable}. Instead we minimise a \textbf{variational upper bound} $L_\theta=-\log p_\theta(x_0)+D_{KL}(q(x_{1:T}|x_0)\,\|\,p_\theta(x_{1:T}|x_0))$ (valid since $\text{KL}\ge0$), which simplifies to $L_{\text{simple}}$.
\end{answerbox}

\q What is $\bar\alpha_t$ and why is it useful?
\begin{answerbox}
$\bar\alpha_t=\prod_{s\le t}(1-\beta_s)$. It lets you jump from $x_0$ to any step $t$ in \textbf{one shot}: $x_t=\sqrt{\bar\alpha_t}\,x_0+\sqrt{1-\bar\alpha_t}\,\epsilon$, so training does not need to simulate the whole chain.
\end{answerbox}

\q Contrast training and sampling in a diffusion model. Why is sampling slow?
\begin{answerbox}
\textbf{Training}: one update samples a \emph{single} random $t$, noises $x_0\to x_t$, and regresses the noise -- cheap, no loop over $t$. \textbf{Sampling}: start from $x_T\sim\mathcal{N}(0,I)$ and run all $T\approx1000$ reverse steps \textbf{sequentially}, each a full network pass -- hence slow. Trap: thinking training also loops over every step.
\end{answerbox}

\q State the generative learning trilemma and where each family sits.
\begin{answerbox}
The three goals are \textbf{high-quality samples}, \textbf{fast sampling}, and \textbf{mode coverage/diversity}; each family gets only \textbf{two}. GANs: quality $+$ speed (sacrifice diversity, mode collapse). Diffusion: quality $+$ diversity (sacrifice speed). VAEs/normalizing flows: speed $+$ diversity (sacrifice quality, blurry).
\end{answerbox}

\q Which generative models have an explicit (or bounded) likelihood, and which does not?
\begin{answerbox}
VAEs and diffusion models optimise a \textbf{likelihood bound} (ELBO / $L_\theta$). GANs have \textbf{no explicit likelihood} -- they are trained implicitly through the adversarial (minimax) game.
\end{answerbox}
